...
この拡張機能には `jj` バイナリは同梱されません。

- **[Jujutsu (jj)](https://github.com/jj-vcs/jj)** - バージョン管理システム
  - ライセンス: Apache License 2.0
  - バージョン管理機能を有効にした場合のみ利用されます
  - GroveNotes はシステム PATH または `grove-notes.versionControl.jjPath` 設定から `jj` を解決します

Jujutsu機能を利用する場合は、以下から `jj` をインストールしてください：
